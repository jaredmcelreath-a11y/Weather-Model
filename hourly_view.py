...
def _wundermap_url(lat: float = KDFW_LAT, lon: float = KDFW_LON,
                   zoom: int = 8) -> str:
    """Wunderground Wundermap URL centered on KDFW with the radar layer on and
    weather stations off. Shows real precipitation reflectivity (not a wind
    overlay). Rendered as an iframe — Wunderground's own map UI, not our styling;
    it may also pull in site chrome or block framing via JS."""
    return (f"https://www.wunderground.com/wundermap?lat={lat}&lon={lon}"
            f"&zoom={zoom}&radar=1&wxstn=0")
...
def render(load_hourly):
    ...
    # Storm radar at the very bottom — the Wunderground Wundermap radar embedded
    # as an iframe (its own map UI). Nothing here can fail the page; if Wunderground
    # blocks framing via JS the iframe may show blank, in which case fall back to
    # the RainViewer radar (kept in git history).
    st.subheader("Radar")
    st.caption("Wunderground radar for the DFW area. Use the map's own controls "
               "to animate and change layers.")
    components.iframe(_wundermap_url(), height=460)
