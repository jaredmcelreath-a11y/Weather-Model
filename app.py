...
Auto-refreshes during the day; switch between Today and Tomorrow in the sidebar.
"""
...
# Minimum model win-probability for the "Safest hold to $1" box. Below this the
# bet isn't low-risk enough to feature regardless of edge (it's a risk cap, not a
...
def load_market(variable: str, day_iso: str):
    """Live Robinhood (ForecastEx) 'Greater than T°' contracts + prices."""
...
def render_variable(col, title, d, variable, day_iso, featured=False,
                    safe_min=SAFE_HOLD_MIN_PROB):
    with col:
        head = f"### {title}"
        if featured:
            head += " ⭐"
        ...
        # Live Robinhood (ForecastEx) ladder vs the model.
        verb = "Greater than" if variable == "high" else "Lower than"
        st.markdown(f"**Live Robinhood market vs model** — “{verb} T°” contracts")
        contracts = load_market(variable, day_iso)
        if not contracts:
            st.caption("No live Robinhood market for this day yet.")
            return
        rows = []
        picks = []  # actionable buys, for the Top-3 section below
        holds = []  # safe hold-to-settlement candidates, for the Safest-hold box
        for c in contracts:
            p = model.prob_for_contract(probs, c["kind"], c["strike"])
...
# --- Layout ---
st_autorefresh(interval=60_000, key="refresh")  # every 60s; market prices cached 30s
...
st.title(f"🌡️  {STATION_ID} Daily High / Low — Market Probabilities")
...
safe_pct = st.sidebar.slider(
    "🛡️ Safe-hold risk floor", min_value=60, max_value=95,
    value=int(SAFE_HOLD_MIN_PROB * 100), step=5, format="%d%%",
...
cols = st.columns(2)
render_variable(cols[0], "High", pred["high"], "high", pred["day"],
                featured=not feature_low, safe_min=safe_min)
render_variable(cols[1], "Low", pred["low"], "low", pred["day"],
...
st.caption("⚠️ Live prices scraped from Robinhood's public event pages (ForecastEx). "
           "High = “Greater than T”, Low = “Lower than T”, settled on the whole-degree "
           "KDFW value. Not financial advice.")
