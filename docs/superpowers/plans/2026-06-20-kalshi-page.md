# Kalshi Market Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a second dashboard page tailored to the Kalshi Dallas temperature market (2°F range buckets), switchable with the existing Robinhood (ForecastEx) page via native Streamlit multipage nav.

**Architecture:** Factor the ~90% shared render logic out of `app.py` into `market_view.py`, parameterized by a small `MarketAdapter` (in `markets.py`) that bundles everything exchange-specific: the live contract fetch, the model→contract price mapping, the on-screen wording, and the safe-hold defaults. `app.py` slims to orchestration + `st.navigation`.

**Tech Stack:** Python 3.9, Streamlit, pandas, pytest. Model + sources unchanged.

---

## File Structure

- **Create `markets.py`** — `MarketAdapter` dataclass + `ROBINHOOD` / `KALSHI` instances + two cached fetch functions.
- **Create `market_view.py`** — all rendering moved out of `app.py` (helpers, `prob_table`, `lock_status`, `reliability_df`, `render_variable`, `render_page`, `_render_accuracy`), parameterized by `adapter`.
- **Create `tests/test_markets.py`** — adapter mapping + safe-hold default tests, and a `market_view` import smoke test.
- **Rewrite `app.py`** — page config, secrets, cached `load_snapshot`/`load_accuracy`, two page functions, `st.navigation(...).run()`.

Unchanged: `model.py`, `sources/*`, `calibration.py`, `forecast_log.py`, `config.py`, `scoring.py`, `backtest.py`, `settlement.py`.

---

## Task 1: Market adapters (`markets.py`)
...
Run: `.venv/bin/python -m pytest tests/test_markets.py -q`
Expected: FAIL with `ModuleNotFoundError: No module named 'markets'`.

- [ ] **Step 3: Create `markets.py`**
...
Run: `.venv/bin/python -m pytest tests/test_markets.py -q`
Expected: PASS (5 passed). Ignore the LibreSSL `NotOpenSSLWarning`.

- [ ] **Step 5: Commit**

```bash
git add markets.py tests/test_markets.py
git commit -m "Add market adapters for Robinhood and Kalshi
...
## Task 2: Shared render module (`market_view.py`)

This moves the rendering out of `app.py` mostly verbatim, parameterized by `adapter`. The only logic changes vs. the current `app.py` are: the market section calls `adapter.fetch` / `adapter.model_prob` / `adapter.heading` / `adapter.no_market_msg`, the "live from Robinhood" caption uses `adapter.name`, and `render_page` reads the safe-hold slider bounds from the adapter and namespaces widget keys by `adapter.name`.

**Files:**
- Create: `market_view.py`
- Test: `tests/test_markets.py` (add an import smoke test)
...
Run: `.venv/bin/python -m pytest tests/test_markets.py::test_market_view_imports_and_exposes_render -q`
Expected: FAIL with `ModuleNotFoundError: No module named 'market_view'`.

- [ ] **Step 3: Create `market_view.py`**
...
```bash
git add market_view.py tests/test_markets.py
git commit -m "Extract shared dashboard render into market_view
...
## Task 3: Wire up the two pages (`app.py`)

**Files:**
- Modify (full rewrite): `app.py`

- [ ] **Step 1: Replace `app.py` with the orchestration version**
...
- [ ] **Step 2: Confirm the whole project still imports and tests pass**

Run: `.venv/bin/python -c "import app, market_view, markets" && .venv/bin/python -m pytest -q`
Expected: imports succeed (no output from the import) and pytest reports all tests passing (existing `test_accuracy.py`, `test_settlement.py`, plus `test_markets.py`).

- [ ] **Step 3: Manual smoke test of both pages**

Run: `.venv/bin/streamlit run app.py` (then open the printed local URL).
Verify:
1. Sidebar shows a page nav with **Robinhood** (default) and **Kalshi**.
2. Robinhood page: title reads "… — Robinhood (ForecastEx)", High/Low columns render with the "Greater than / Lower than T°" contract table and the footer mentions Weather Underground.
3. Switch to **Kalshi**: title reads "… — Kalshi (Kalshi)", the contract table shows 2°F buckets (e.g. "90° to 91°"), the safe-hold slider defaults to 55%, and the footer mentions the NWS CLIDFW report.
4. The Day toggle and safe-hold slider work independently on each page (no cross-page key collision errors).
...
```bash
git add app.py
git commit -m "Add Kalshi page via st.navigation; app.py orchestrates both markets
...
- **Spec coverage:** `markets.py` (adapters incl. Kalshi safe-hold 55%/50% and CLIDFW footer) → Task 1; `market_view.py` (shared render, `adapter.fetch`/`model_prob`/`heading`, namespaced keys) → Task 2; `app.py` `st.navigation` two pages → Task 3; adapter mapping + import tests → Tasks 1–2; manual UI verification → Task 3 Step 3. Existing `model`/`sources` untouched (spec "stays identical").
- **Type consistency:** `render_variable(col, title, d, variable, day_iso, adapter, featured, safe_min)` and `render_page(snap, calib, adapter, load_accuracy)` signatures are used consistently in `app.py`. Adapter attribute names (`fetch`, `model_prob`, `heading`, `no_market_msg`, `settle_footer`, `safe_hold_default`, `safe_hold_min`, `name`, `exchange`) match between `markets.py` definitions and `market_view.py`/`app.py` uses.
